...
'''
** this version can be (optionally) geared up n times such that the gear 
ratio is (R/r)^(n-1). Else it assumes n = 5.
'''
import numpy as np
...
parser.add_argument("-s", "--specs", action="store_true", help="using this option prints important \
	information about the given specs on the alternator \
	used to calcuate the outputs of this script")
args = parser.parse_args()

def alternator_data(P): #the data in this function is a placeholder!
	if P == args.P:
		P_mech = P / efficiency
...
if args.specs:
	coefficients = alternator_data(None)
	...
	if efficiency <= 0:
		print "not a valid power input for this generator. Try option -s for more info."
	else:
		unpack = alternator_data(args.P)
		coefficients = unpack[1]
		P_mech = unpack[0]
		omega = 0.
